      ...
      <form onSubmit={(e)=>e.preventDefault()}>
        ...
        <div className="form-control">
          ...
          <div>
            {categories.map((c,index)=>{
              return(
                <button type='button' key={index} onClick={updateFilters} name="category" className={`${category === c.toLowerCase()? 'active' : null}`}>{c}</button>
              )
            })}
          </div>
        </div>
        ...
        <div className="form-control">
          ...
          <div className="colors">
            {colors.map((c,index)=>{
              if(c ==='all'){
                return <button type="button" key={index} name="color" onClick={updateFilters} data-color="all" className={`${color === 'all' ? 'all-btn active': 'all-btn'}`}>all</button>
              }
            return(
              <button type="button" name="color" key={index} className={`${color ===c ? 'color-btn active' : 'color-btn' }`} style={{backgroundColor:c}} data-color={c} onClick={updateFilters}>
